...
function mapLeadInput(input: unknown): LeadPayload {
  ...
  return {
    ...
    bridePhone: parseNullableString(raw.bride_phone ?? raw.bridePhone),
    groomPhone: parseNullableString(raw.groom_phone ?? raw.groomPhone),
    brideEmail: parseNullableString(raw.bride_email ?? raw.brideEmail),
    groomEmail: parseNullableString(raw.groom_email ?? raw.groomEmail),
    address1: parseNullableString(raw.address1 ?? raw.addr1),
    address2: parseNullableString(raw.address2 ?? raw.addr2),
    interests: interests ?? undefined,
    weddingPlannedOn: parseDateOnlyString(raw.wedding_planned_on ?? raw.weddingPlannedOn),
    expectedVenue: parseNullableString(raw.expected_venue ?? raw.expectedVenue),
    memo: parseNullableString(raw.memo),
    source,
    visited: parseBoolean(raw.visited),
    consent: parseBoolean(raw.consent),
    consentAt: parseDate(raw.consent_at ?? raw.consentAt),
    customerId: parseNullableString(raw.customer_id ?? raw.customerId),
  };
}
